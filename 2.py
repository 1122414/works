...
class MQ:
    ...
    def mqinit(self,ip, port, username, password, host):
    ...
    #     credentials = pika.PlainCredentials(username, password)
    #     connection = pika.BlockingConnection(pika.ConnectionParameters(ip, port, '/test_4', credentials))
    #     return connection.channel()
    ...
    def mqSend(self):
        ...
        while send_len:
            ...
            try:
                ...
                # 发布消息到队列
                self.sendMQ.basic_publish(exchange='scrapy', routing_key=routing_key, body=message)
...
if __name__ == "__main__":
    ...
    site = {
            "domain": "https://lakbase.io",  # 
            "platform": "[]",  #
            "url": "https://lakbase.io/", #
            "content_encode": "utf-8", #
            "title": "test_1", #
            "site_name": "test_1", #
            "index_url": "https://leakbase.io", #
            "description": "[]", #
            "snapshot": "{}", #
            "name": "", #
            "path": "", #
            "image_hash": "", #
            "last_status": "online", #
            "first_publish_time": "2000-01-01 00:00:00", #
            "last_publish_time": "2000-01-01 00:00:00", #
            "service_type": "\u8bba\u575b", #
            "is_recent_online": "online", #
            "scale": "{}", #
            "active_level": "[]", #
            "label": "[]", 
            "goods_label": "[]", #
    ...
    message = json.dumps(site)

    mq.sendMQ.basic_publish(exchange='scrapy', routing_key="site", body=message)
